...
def build_system_prompt(schema_json: str|None, tone: str) -> str:
    tone_line = "Tone: executive-friendly, concise." if tone=="exec" else "Tone: hands-on operator, step-by-step."
    if schema_json:
        return (
    ...
    return (
        ...
        '{"executive_summary": str, "sections": [{"title": str, "bullets": [str], "metrics_highlights": {}}], '
...
def synthetic_label(findings: Dict[str,Any], actions: List[Dict[str,Any]]) -> Dict[str,Any]:
    # Create a minimal, valid Insight label. Replace with real editor-reviewed labels as they become available.
    account_name = (findings.get("account",{}) or {}).get("account_name") or findings.get("account",{}).get("account_id","account")
    date_range = findings.get("date_range",{})
    dr = f'{date_range.get("start_date","")} to {date_range.get("end_date","")}'
    summary = f"[AUTO] Summary for {account_name} ({dr}). {len(actions)} actions proposed based on rules."
    sections = [
