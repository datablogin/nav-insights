...
class LlamaCppClient:
    ...
    def generate_structured(self, schema_model: Type[T], system_prompt: str, user_prompt: str,
                            max_tokens: int = 800, temperature: float = 0.2,
                            retries: int = 2, try_json_schema_mode: bool = True) -> T:
        ...
        if try_json_schema_mode:
            try:
                content = self._chat(messages, max_tokens=max_tokens, temperature=temperature,
        ...
        fallback = [
            {"role":"system","content":"Return ONLY minified JSON that validates the provided schema. No prose.\n" +
        ...
        for _ in range(retries+1):
            ...
            try:
                ...
                last_err = ve
                fallback.append({"role":"system","content":f"Previous JSON failed validation: {ve}. Return corrected JSON only."})
                time.sleep(0.1)
