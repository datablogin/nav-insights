...
class TestDSLExpressions:
    ...
    def test_and_operator(self, sample_data):
        ...
        assert eval_expr("value('flags.has_data') and value('metrics.spend') > 0", sample_data) is True
    ...
    def test_or_operator(self, sample_data):
        ...
        assert eval_expr("value('flags.is_active') or value('metrics.clicks') > 0", sample_data) is True
...
class TestRegisteredHelpers:
    ...
    def test_helpers_in_complex_expression(self):
        ...
        expr = "pct(value('conversion_rate')) + ' conversion rate generates ' + usd(value('revenue'))"
